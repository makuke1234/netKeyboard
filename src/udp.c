...
static inline DWORD WINAPI s_udpThread_readKbdOut_thread(LPVOID lpParams)
{
	udpThread_t * uThread = lpParams;
	assert(uThread != NULL);
...
bool udpThread_readKbdOut(udpThread_t * restrict uThread, void * restrict buffer, int bufSize)
{
	uThread->killSwitch = false;
	uThread->hasIncome  = false;
	uThread->buffer     = buffer;
